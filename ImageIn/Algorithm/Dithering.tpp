...
namespace imagein {
	namespace algorithm {
		...
		template <typename D>
		GrayscaleImage_t<D>* Dithering_t<D>::algorithm(const std::vector<const Image_t<D>*>& imgs)
		{
					...
					for(unsigned int l = 0 ; l < d->height ; ++l) {
						for(unsigned int k = 0 ; k < d->width ; ++k) {
							...
							try {
								...
								if(coef > 0) {
									D oldValue = img->getPixel(i+k-(d->center), j+l);
									...
									img->setPixel(i+k-(d->center), j+l, newValue);
