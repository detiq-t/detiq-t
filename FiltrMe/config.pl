...
##############
  ...
  #Search pathes for genericinterface
  #INCLUDES
  
  if(`ls -a .config_Igi 2> /dev/null` eq "")
  {
    `find /usr -name "GenericInterface.h" >> .tmp 2> /dev/null`;
    `find /home -name "GenericInterface.h" >> .tmp 2> /dev/null`;
  ...
  if(`ls -a .config_Lgi 2> /dev/null` eq "")
  {
    ...
    while($line = <TMP>)
    {
      if($line =~ "(.*/)libQtInterface.a")
      {
        ...
        print "libQtInterface.so found: $pathLQtI\n";
        last;
      }
    }
  ...
  while($line = <MAKEFILE>)
  {
    ...
    if($oldline =~ "^INCPATH")
    {
      if(!($line =~ "^INCPATH"))
      {
        ...
        print NMAKEFILE "INCPATH       += -I$pathIQtI\n";
        print "added line: INCPATH       += -I$pathIQtI\n";
      }
    }
